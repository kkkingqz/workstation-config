...
**Итог (2026-09-26), фаза закрыта с отступлениями:**

- Вход по suspend не выполнен: 3 чистых цикла из 20; проверку пропустили
  (решение пользователя). Нестабильный suspend после миграции сначала
  сравнивать с этим числом, а не считать регрессией Nix.
- Recovery (шаг 11) на деле не загружали: снимок обновлён скриптом,
  `grub-script-check` чистый, ядро recovery `7.2.7-1-t2`.
- Эталон снят с `--with-sudo` (`dpkg --verify`, initrd, контрольная сумма
  `refind.conf`) — шаг 1 фазы 0 уже выполнен.
- `ws-gnome-test` в эталоне содержит FAIL «legacy extension still
  installed: window-monitor-pro»: противоречит verify 10.1, где расширение
  обязательно. Состояние до миграции, в diff не выйдет; решить отдельно.
- `/mnt/apple` удалён из fstab; пароль root не задан.
...
отдельно). CI зелёный (run 36256163512: `nix flake check` собирает
поколение, синтаксис скриптов); тег `nix-phase-0` на `85f70bf`.
